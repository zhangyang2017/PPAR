...
```{r}
library(knitr)
.cran_packages <- c("ggplot2", "dplyr", "tidyverse", "ggpubr", "ggrepel", "lsmeans", "car", "gridExtra", "export")
...
feeding <- read.csv(file = "/Users/yangzhang/Box/MAN/pub/figure1/phenome_feeding.csv", as.is = TRUE)
feeding <- feeding %>%
...
## measurements
dexa <- read.csv(file = "/Users/yangzhang/Box/MAN/figure1/phenome_metabolic_assessment.csv", as.is = TRUE)
dexa <- dexa %>%
  dplyr::mutate(vars = factor(Treatment,
                                   levels = c("HFD", "HFD+LXN", "HFD+HXN",
                                              "HFD+TXN", "LFD")))
...
## metabolic cage data
profile <- read.csv(file = "/Users/yangzhang/Box/MAN/figure1/metabolic_assessment.csv", as.is = TRUE)
...
bars <- read.csv(file = "/Users/yangzhang/Box/MAN/figure1/phenome_bars.csv", as.is = TRUE)
...
H <- ggplot(data = dexa, aes(x = vars, y = bat_wt, fill = vars)) +
       stat_summary(fun.data = pars, geom = "boxplot", position = "dodge") +
       stat_summary(fun.y = outliers, geom = "point") +
       scale_fill_manual(values = alpha(mycolor2, .6)) +
       xlab("") + ylab("g") +
...
I <- ggplot(data = dexa, aes(x = vars, y = visceral_wt, fill = vars)) +
       stat_summary(fun.data = pars, geom = "boxplot", position = "dodge") +
       stat_summary(fun.y = outliers, geom = "point") +
       scale_fill_manual(values = alpha(mycolor2, .6)) +
       xlab("") + ylab("g") +
...
K <- ggplot(data = dexa, aes(x = vars, y = liver_wt, fill = vars)) +
       stat_summary(fun.data = pars, geom = "boxplot", position = "dodge") +
       stat_summary(fun.y = outliers, geom = "point") +
       scale_fill_manual(values = alpha(mycolor2, .6)) +
       xlab("") + ylab("g") +
...
L <- ggplot(data = dexa, aes(x = vars, y = LV_TAG, fill = vars)) +
       stat_summary(fun.data = pars, geom = "boxplot", position = "dodge") +
       stat_summary(fun.y = outliers, geom = "point") +
       scale_fill_manual(values = alpha(mycolor2, .6)) +
       xlab("") + ylab("mg/g") +
...
partB <- ggarrange(E, F, G, H, I, J, K, L, align = "hv",
                   labels = c("E", "F", "G", "H", "I", "J", "K", "L"),
                   font.label = list(size = 22, color = "black"),
